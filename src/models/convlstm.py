...
class DilatedNet(nn.Module):
    def __init__(self, n_steps_past=16, num_inputs=4,
        ...
        self.lstm_dim=h6
        self.hidden_units = [h1,h2,h3,h4,h5]
        ...
        ## LSTM
        self.lstm = nn.LSTM(input_size=self.hidden_units[2],
                         hidden_size=self.hidden_units[3],
                         num_layers=self.lstm_dim,
                         bidirectional=True)

        ## Linear part of the network
        self.lin1 = nn.Linear(in_features=(self.hidden_units[3]*2*self.length3), # self.lstm_dim*(self.hidden_units[3]*2  #skal være self.hidden_units[3] da det er output fra self.lstm_CHO_insulin
                    out_features=self.hidden_units[4],
                    bias=False)

        self.lin2 = nn.Linear(in_features= self.hidden_units[4], # self.lstm_dim*(self.hidden_units[3]*2  #skal være self.hidden_units[3] da det er output fra self.lstm_CHO_insulin
                    out_features=1,
                    bias=False)
    ...
    def forward(self, x):
        ...
        x = self.max_pool(x)
        # print('out max 1: ',x.shape)

        x = self.relu(self.conv2(x))
        # print('out conv 2: ',x.shape)
        x = self.max_pool(x)
        # print('out max 2: ',x.shape)

        x = self.relu(self.conv3(x))
        # print('out conv 3: ',x.shape)
        x = self.max_pool(x)
        # print('out max 3: ',x.shape)
        x = self.dropout(x)
        ...
        # print('in linear 1 : ',x.shape)
        # features_final = self.relu(x)
        features_final = self.relu(self.lin1(x))
        x = self.dropout(x)
        # print('in linear 2 : ',features_final.shape)
        out = self.lin2(features_final)
        # print('out linear 2 : ',out.shape)
