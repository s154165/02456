...
from config_convlstm import code_path, data_path
...
def searchBestHypers(num_samples=10, max_num_epochs=15, n_epochs_stop=2, grace_period=5, gpus_per_trial=0, data_obj=None):
    ...
    config_schedule = {
        ...
        "h4": tune.sample_from(lambda: 2 ** np.random.randint(0, 4)), # LSTM hidden
        "h5": tune.sample_from(lambda: 2 ** np.random.randint(3, 8)), # linear output
